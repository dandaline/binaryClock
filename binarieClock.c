/**
 * @file binarieClock.c
 * @author Derya Kilicarslan, Laila Staschenuk
 * @brief a schoolproject to make a binary clock using charlieplexed LEDs, three buttons and ATtiny461A
 * @version 1.0
 * @date 2023-01-23
 *
...
// Global variables to set hours, minutes and seconds
volatile uint8_t hours = 3;
volatile uint8_t minutes = 16;
volatile uint8_t seconds = 0;
volatile uint8_t ledSeq[11] = {0};

// Global variable for debouncing button and sleep mode
...
/**
 * @brief inits the microcontroller
 *
 */
void setup()
{
	...
	TCCR1B |= (1 << CS13); // set Timer1(Time) auf f/128 bei f= 2^15 overflow after 1s
	TIMSK |= (1 << TOIE1); // set interrupts for Timer1

	// Pin Change Interrupts - enable PCINT0, PCINT11 and PCINT14
	...
	PCMSK1 |= tastMinSleep;				  // enable PCINT14 and PCINT11 on PORTB PIN B6
...
void updateSeq()
{
	...
	// disregards actual time, in case actual time is 00:00
	uint16_t time = 0b0;
...
/**
 * @brief changes time values, is called by ISRs
 * @details when seconds are == 60, update LEDSeq && increment hours, minutes
...
/**
 * @brief Gets called every second
 * @details timer overflow interrupt takes one second, increment seconds when overflow
 ...
 * 
 * else: show Time Part (Pseudo PWM)
